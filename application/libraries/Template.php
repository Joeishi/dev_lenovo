...
class CI_Template {
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template']) 
         && (file_exists(APPPATH .'views/'. $props['folder'] .'/'. $props['template']) or file_exists(APPPATH .'views/'. $props['folder'] .'/'. $props['template'] . EXT)))
      {
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      $args = func_get_args();
      $view = $this->template['folder'] .'/'. $view;
      ...
      if (count($args) > 1)
      {
         ...
         {
            if (file_exists(APPPATH .'views/'. $this->template['folder'] .'/'. $suggestion . EXT) or file_exists(APPPATH .'views/'. $this->template['folder'] .'/'. $suggestion))
            {
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
         ...
         else
         {
			...
			$master = $this->template['folder'] .'/'. $this->master;
            $output = $this->CI->load->view($master, $this->output, $buffer);
         }
      }
